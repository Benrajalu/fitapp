  ...
  render() {
    ...
    if(trueExercise){
      ...
      if(status){
        ...
        repTarget = currentExercise.repTarget ? currentExercise.repTarget : currentExercise.handicap;
        ...
        <div className="routine-log">
          <div className="description">
            <div className="header">
              ...
              {cleanType ? 
                <p>{cleanType} {bodyPart ?  <span>• {bodyPart}</span> : false} </p> 
                : 
                false
              }
            </div>
            <div className="objectives">
              <p>Objectifs</p>
              <div className="units">
                <p className="sets"><strong>{setNumber < 10 ? "0" : false}{currentExercise.sets ? setNumber : "01"}</strong> set{ setNumber > 1 ? "s" : false}</p>
                {currentExercise.repTarget || currentExercise.reps ? <p className="reps"><strong>{repTarget < 10 ? "0" : false}{repTarget}</strong> rep{ repTarget > 1 ? "s" : false}</p> : false}
                {currentExercise.handicap ? <p className="handicap"><strong> {currentExercise.handicap}</strong> {unit}</p> : false}
              </div>
            </div>
          </div>
          {status ?
            <div className="report">
              <div className="progress-bar">
                <div className="progress" style={{"width": completedPercentage + "%"}}></div>
              </div>
              <p><strong>{completedSets}/{currentExercise.sets.length}</strong> <span>–</span> sets complétés</p>
            </div>
            :
            false
          }
