...
class WorkoutsHistory extends Component {
  render() {
    ...
    const output = workoutItems.map((value, index) => 
      <WorkoutHistoryDetail key={value.id} contents={value} exercisesDatabase={this.props.exercisesDatabase} delay={index * 100}/>
    )
      ...
      <div className="workout-logs-list">
        {workoutItems.length > 0 ? 
          ...
          <div className="empty-workouts">
            ...
            <button className="btn btn-ghost"><FontAwesomeIcon icon={['fas', 'play']} size="1x" /> Lancer une scéance</button>
          </div>
        }
      </div>
    )
