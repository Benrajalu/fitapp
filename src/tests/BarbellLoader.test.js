...
describe('given set discs and target weights', () => {
  ...
  const loaderThirtyFive = mount(<BarbellLoader settings={settings} weight={35} />);
  ...
  test('it displays smaller discs first', () => {
    expect(loaderFourty.find('.disc').first().text()).toEqual('5 kg');
    expect(loaderFourty.find('.disc').at(1).text()).toEqual('10 kg');
  });
});